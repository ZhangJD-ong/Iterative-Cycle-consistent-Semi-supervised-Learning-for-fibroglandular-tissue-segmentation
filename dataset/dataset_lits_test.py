...
class Img_DataSet(Dataset):
    def __init__(self, x0,x1, cut_param):
        self.x0 = x0
        self.x1 = x1
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(pre).type(torch.float32),torch.from_numpy(pos).type(torch.float32)
...
def label_coding(gt, label_num):
    new_gt = np.zeros([label_num, gt.shape[0], gt.shape[1], gt.shape[2]])
...
def Test_all_Datasets(dataset_path,size):
    ...
    for file in data_list:
        print("\nStart Evaluate: ", file)
        x1,x1_min,x1_max = normalization(load(os.path.join(dataset_path,'w_label',file,'P1_reg.nii.gz')))
        x1 = x1.astype(np.float32)
        x0 = normalization_fix(load(os.path.join(dataset_path,'w_label',file,'P0.nii.gz')),x1_min,x1_max)
        x0 = x0.astype(np.float32)
        gt = load(os.path.join(dataset_path,'w_label',file,'Tissues.nii.gz')).astype(np.float32)
        new_gt = np.zeros(gt.shape).astype(np.float32)
        ff = np.where(gt==2)
        new_gt[ff]=1
        breast = load(os.path.join(dataset_path,'w_label',file,'Breast.nii.gz')).astype(np.float32)
        breast_erode = sitk.GetArrayFromImage(sitk.BinaryErode(sitk.ReadImage(os.path.join(dataset_path,'w_label',file,'Breast.nii.gz')),(2,2,2))).astype(np.float32)

        itkimage = sitk.ReadImage(os.path.join(dataset_path,'w_label',file, 'P0.nii.gz'))
        original_shape = gt.shape
        new_shape = cal_newshape(gt,size)

        yield Img_DataSet(x0,x1,size),original_shape, new_shape,itkimage, file,new_gt,breast,breast_erode
